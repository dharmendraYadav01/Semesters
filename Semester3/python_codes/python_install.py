#check the python version installed on your system through commandline.
#WAP to print the ASCII vlaue of a character{hint-ord{}function}
#Wap to find diffenence of two small integers(not greatre then 10)without using minus(-)operator.[hint-use ^operator].
#WAP to multipy a given no.by 2 without using*(multiply operator)
#WAP to calculate and display the interest on a loan amount(Rupees).You would need three variable to hold 'principal',Rate_of_interest'(%)
...
#WAPP that accept an integer(n) and computes the value of n+nn+nnn.
#take a 5 digit no. as a input and reverse it without using loop.
#Two no.are input through the keyboard into two locations C and D.Write a programm to interchange the contests of C and D.
#if a four digit no. is input through the keyboard, write a program to obtain the sum of first and last digit of this no.
#in a town.the percentage of men is 52.The percentage of total literacy is 48.if total percentage of literate men 35 of the total population
#write a programm to find a total no. of illiterate men and women if the total population of town is 80,000.
#A cashier has a currency note denomination 10,50 & 100, If the amount to be withdrawn is input through the keybord is hundreds, find the total number of currency notes of each denomination the cashier is here to give the withdrawer
